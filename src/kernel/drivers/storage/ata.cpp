...
Ata::Ata(Bus bus, Port port, Mode mode) : port_(port) {}

void Ata::read_sectors(std::byte* buffer, size_t offset, size_t amount) {
    send_amount(amount);
...
void Ata::send_amount(size_t amount) {
    io::write_byte(io::Port::PRIMARY_ATA_SELECTOR_COUNT, amount);
}

void Ata::send_sector_offset(size_t offset) {
...
void Ata::read_sector(std::byte* buffer) {
